...
function import_data_field_validation($value = NULL, $request = NULL, $param = NULL) {
  ...
  if($value == 'process') {
    ...
    foreach($clients as $key => $item) {
      ...
      $user_id = hdm_get_hdclient_user_id($item['idclienti'], $item);
      $debug .= "\nuser_id $user_id";
      ...
      if ( $user_id ) {
        ...
        $userdata = array(
          ...
          'display_name'          => ((!empty($item['soprannome'])) ? $item['soprannome'] . ' ' : '') . $item['displayname'],   //(string) The user's display name. Default is the user's username.
